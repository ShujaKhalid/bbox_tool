...
class App extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      header: [],
      segClass: 0,
      localKey: 0, // potential draggable element 
      globalKey: 0,      
  ...
  onMaskP = (maskVars) => {
	...
	//console.log('this.state.globalKey: ' + this.state.globalKey)
	//console.log('this.state.localKey: ' + this.state.localKey)
  ...
  undo = () => {
    if (this.state.lastClick[this.state.lastClick.length-1]==='rec') {
      ...
      this.state.maskInd = this.state.maskP.length-1
      this.state.resCoords.pop()

      // Modify the localKey and/or the globalKey
      if (this.state.localKey==0) {
      	if (this.state.globalKey!=0) {
      	  this.state.globalKey = this.state.globalKey-1
      	  this.state.localKey = this.state.defaultPosition[this.state.globalKey].length-1-1
      	}
      } else {
      	this.state.localKey = this.state.localKey-1
      }
  ...
  onEnterDown = () => {
  	this.setState({
  		...
  		localKey: 0, // global key gets incremented and local key is reset
  	})
  }
  ...
  dispResult = () => {
	...
	for (var j=0; j<=this.state.globalKey; j+=1) {
	  	for (var i=0; i<this.state.defaultPosition[j].length; i+=1) {
	   	...
	   	resClass.push(
	   	  <tr>	
	   	    <select key={this.state.resClass.length+1}>
