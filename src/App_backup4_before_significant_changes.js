...
class App extends Component {
  ...
  onMaskP = (maskVars) => {
    ...
    this.state.defaultPosition.push(this.state.maskP[this.state.maskP.length-1]) 
  ...
  handleDrag = (e, ui) => {
    const key = e.toElement.classList[0]
  ...
  dispResult = () => {
  	...
  	result.push(<tr key={0}>
	    <td>No. </td>
	    <td>Coord. X</td>
	    <td>Coord. Y</td>
	    <td>Segment Class (0-7)</td>
	  </tr>)
  	for (var i=1; i<=this.state.defaultPosition.length; i+=1) {
	  result.push(<tr key={i}>
	    <td>{i}</td>
	    <td>{this.state.defaultPosition[i-1].x}</td>
	    <td>{this.state.defaultPosition[i-1].y}</td>
	  </tr>)
    }
   	result.push(
...
class ImgPic extends Component {
  ...
  updateCanvas = () => {
    ...
    //console.log('inside where i need to be')
    ...
    if (this.isDragPoly) {
      ...
      this.ctx.beginPath();
      //this.ctx.moveTo(this.props.polyP[0].x, this.props.polyP[0].x)
  ...
  onMouseUp = (e) => {
    ...
    if (e.target.tagName==='CANVAS' || e.target.id==='circle') {
