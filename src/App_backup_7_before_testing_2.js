	...
	if (this.state.lastClick[this.state.lastClick.length-1]==='rec') {
	  ...
	  if (this.state.localKey==0) {
		if (this.state.globalKey!=0) {
		  ...
		  this.state.localKey = this.state.defaultPosition[this.state.globalKey].length-1
		}
	  } else {
	  ...
	  buttons: [
		{
		  ...
		  onClick: () => {
			...
			for (let i=0;i<this.props.polyP.length;i++) {
				if (this.props.polyP[i].length!=0) {
					bboxLen+=1
				}
			}
			console.log(bboxLen)
